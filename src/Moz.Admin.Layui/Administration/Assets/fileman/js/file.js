...
function File(filePath, fileSize, modTime, w, h) {
    ...
    this.height = (h ? h : 0);
    this.Show = function () {
        html = '<li data-path="' + this.fullPath + '" data-time="' + this.time + '" data-icon="' + this.icon + '" data-w="' + this.width + '" data-h="' + this.height + '" data-size="' + this.size + '" data-icon-big="' + (this.IsImage() ? this.fullPath : '/Administration/Assets/fileman/' + this.bigIcon) + '" title="' + this.name + '">';
        //if (this.isMultiImage()) {
        //    html += '<input type="checkbox" name="multiimage" />';
        //}
        html += '<img src="' + this.icon + '" class="icon" style="background-image:url(&quot;xfinder.ashx?a=GENERATETHUMB&f=' + this.fullPath + '&width=140&height=120&quot;);">';
        //html += '<span class="time">' + RoxyUtils.FormatDate(new Date(this.time * 1000)) + '</span>';
        html += '<span class="name" style="display:block;">' + this.name + '</span>';
    ...
    this.isMultiImage = function () {
        return RoxyUtils.GetUrlParam('multiimage') === "yes";
    }
}
