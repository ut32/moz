...
function Directory(fullPath, numDirs, numFiles) {
    ...
    this.SetEvents = function () {
        ...
        el.click(function (e) {
            ...
            var requested = $li.attr("requested");
            if (requested === "1") {
                selectDir(obj);
            } else {
                $li.attr("requested", "1");
                ...
                $.ajax({
                    url: 'fileBrowser/dirList?path=' + path,
                    type: 'POST',
                    dataType: 'json',
                    async: false,
                    cache: false,
            ...
            if (requested === "1") {
                ...
                $.ajax({
                    ...
                    success: function (dirs) {
                        ...
                        var lid = Directory.Parse($li.attr('data-path'));
                        lid.Expand();
                    }
                });
                //
    ...
    this.GetHtml = function () {
        ...
        if (this.dirs > 0) {
            html += '<div><img src="/Administration/Assets/fileman/images/dir-plus.png" class="dirPlus" width="9" height="9" path="' + this.name + '">';
        } else {
            html += '<div><img src="/Administration/Assets/fileman/images/blank.gif" class="dirPlus" width="9" height="9">';
        }
        html += '<img src="/Administration/Assets/fileman/images/folder.png" class="dir"><span class="name">' + this.name + '(' + this.files + ')</span></div>';
    ...
    this.SetOpened = function () {
        ...
        if (li.find('li').length < 1)
            li.children('div').children('.dirPlus').prop('src', '/Administration/Assets/fileman/images/blank.gif');
        else if (this.IsExpanded())
            li.children('div').children('.dirPlus').prop('src', '/Administration/Assets/fileman/images/dir-minus.png');
        else
            li.children('div').children('.dirPlus').prop('src', '/Administration/Assets/fileman/images/dir-plus.png');
    };
