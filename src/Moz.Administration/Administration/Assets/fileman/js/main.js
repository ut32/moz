...
function makeDragFile(e) {
    ...
    return '<div class="pnlDragFile" data-path="' + f.fullPath + '"><img src="/Administration/Assets/fileman/' + f.bigIcon + '" align="absmiddle">&nbsp;' + f.name + '</div>';
}
function makeDragDir(e) {
    ...
    return '<div class="pnlDragDir" data-path="' + f.fullPath + '"><img src="/Administration/Assets/fileman/images/folder.png" align="absmiddle">&nbsp;' + f.name + '</div>';
}
...
function getSelectedFiles() {
    var rets = [];
    $('#pnlFileList li.selected').each(function (ele, idx) {
        rets.push(new File($(this).attr('data-path')))
    });
    return rets;
}
function selectAllFile() {
    if ($('#pnlFileList li.selected').size() === $('#pnlFileList li').size()) {
        $('#pnlFileList li').removeClass('selected');
    } else {
        $('#pnlFileList li').addClass('selected');
    }
}
...
function switchView(t) {
    ...
    if (t == 'thumb') {
        $('#pnlFileList .icon').attr('src', '/Administration/Assets/fileman/images/blank.gif');
        ...
        if ($('#dynStyle').length == 0) {
            ...
            rules += 'ul#pnlFileList.thumbView li{height:' + (parseInt(RoxyFilemanConf.THUMBS_VIEW_HEIGHT) + 40) + 'px;}';
...
function ResizeLists() {
    var tmp = $(window).innerHeight() - 45;
    $('.scrollPane').css('height', tmp);
    $('.pnlFiles .scrollPane').css('height', $(window).innerHeight() - 104);
}
...
function getMultiImage() {
    var multiimage = RoxyUtils.GetUrlParam('multiimage');
    return multiimage === "yes";
}
function getCallback() {
    return RoxyUtils.GetUrlParam('callback');
}
function setFile() {
    var f = getSelectedFile();
    if (!f) {
    ...
    switch (getFilemanIntegration()) {
        case 'ckeditor':
            if (getMultiImage()) {
                var fs = getSelectedFiles();
                if (fs.length <= 0) {
                    window.opener.insertPathToCkeditor(insertPath);
                } else {
                    window.opener.insertPathToCkeditor(fs.map(function (value) { return value.fullPath }).join(','));
                };
            } else {
        ...
        case 'ueditor':
            ...
            if (fs.length <= 0) {
                ...
                if (getMultiImage()) {
                    window.opener.uEditorCallBack(fs.map(function (value) { return value.fullPath }).join(','));
                } else {
                    window.opener.uEditorCallBack(fs[0]);
                }
            }
            self.close();
            break;
        ...
        case 'input':
            ...
            if (fs.length <= 0) {
                ...
                if (getMultiImage()) {
                    window.opener[getCallback()](fs.map(function (value) { return value.fullPath }).join(','));
                } else {
                    window.opener[getCallback()](fs[0].fullPath);
                }
            }
            self.close();
            break;
